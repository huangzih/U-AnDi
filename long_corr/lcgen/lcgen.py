...
class FBM(object):
    ...
    def __init__(self, n, A, hurst,tau, omega,corr_type, length=1, method="daviesharte"):
        """Instantiate the FBM."""
    ...
    def fgn(self):
        ...
        if self.hurst == 0.5 and self.corr_type!="ml" and self.corr_type!="expo" and self.corr_type!="multiexp" and self.corr_type!="expcos":
            return gn * scale
        else:
            fgn = self._fgn(gn)
    ...
    def _autocovariance(self, k):
        ...
        allowed_corr_type = ["pl","ml","expo","multiexp","expcos"]
        assert self.corr_type in allowed_corr_type, f"corr_type must be in these values: {', '.join(allowed_corr_type)}"
        if self.corr_type=="pl":
            return 0.5 * self.A * (abs(k - 1) ** (2 * self.hurst) - 2 * abs(k) ** (2 * self.hurst) + abs(k + 1) ** (2 * self.hurst))
        elif self.corr_type=="ml":
            return self.A * ml(-(abs(k)/self.tau)**(2*self.hurst),2*self.hurst)/(self.tau ** (2*self.hurst))
        elif self.corr_type=="expo":
            return self.A * np.exp(-abs(k)/self.tau)
        elif self.corr_type=="multiexp":
            return sum([0.5 * self.A * np.exp(-abs(k)/self.tau[i]) for i in range(len(self.tau))])
        elif self.corr_type=="expcos":
            return self.A * np.exp(-abs(k)/self.tau)*np.cos(self.omega*abs(k))
...
def fbm_pl(n, A, hurst, length=1, method="daviesharte"):
    """One off sample of fGn."""
    f = FBM(n=n, A=A, hurst=hurst, tau=10, omega=0.1, corr_type="pl", length=length, method=method)
    return f.fbm()

def fbm_ml(n, A, hurst, tau, length=1, method="daviesharte"):
    """One off sample of fGn."""
    f = FBM(n=n, A=A, hurst=hurst, tau=tau, omega=0.1, corr_type="ml", length=length, method=method)
    return f.fbm()

def fbm_expo(n, A, tau, length=1, method="daviesharte"):
    """One off sample of fGn."""
    f = FBM(n=n, A=A, hurst=0.5, tau=tau, omega=0.1, corr_type="expo", length=length, method=method)
    return f.fbm()

def fbm_multiexp(n, A, tau, length=1, method="daviesharte"):
    """One off sample of fGn."""
    assert type(tau)==list,"type of `tau` must be list in multi-exp"
    f = FBM(n=n, A=A, hurst=0.5, tau=tau, omega=0.1, corr_type="multiexp", length=length, method=method)
    return f.fbm()

def fbm_expcos(n, A, tau, omega, length=1, method="daviesharte"):
    """One off sample of fGn."""
    f = FBM(n=n, A=A,hurst=0.5, tau=tau, omega=omega,corr_type="expcos", length=length, method=method)
    return f.fbm()


def fgn_pl(n, A, hurst, length=1, method="daviesharte"):
    """One off sample of fGn."""
    f = FBM(n=n, A=A, hurst=hurst, tau=10,omega=0.1, corr_type="pl", length=length, method=method)
    return f.fgn()

def fgn_ml(n, A, hurst, tau, length=1, method="daviesharte"):
    """One off sample of fGn."""
    f = FBM(n=n, A=A, hurst=hurst, tau=tau, omega=0.1, corr_type="ml", length=length, method=method)
    return f.fgn()

def fgn_expo(n, A, tau, length=1, method="daviesharte"):
    """One off sample of fGn."""
    f = FBM(n=n, A=A, hurst=0.5, tau=tau, omega=0.1, corr_type="expo", length=length, method=method)
    return f.fgn()

def fgn_multiexp(n, A, tau, length=1, method="daviesharte"):
    """One off sample of fGn."""
    assert type(tau)==list,"type of `tau` must be list in multi-exp"
    f = FBM(n=n, A=A, hurst=0.5,tau=tau, omega=0.1, corr_type="multiexp", length=length, method=method)
    return f.fgn()

def fgn_expcos(n, A, tau, omega, length=1, method="daviesharte"):
    """One off sample of fGn."""
    f = FBM(n=n, A=A, hurst=0.5, tau=tau, omega=omega,corr_type="expcos", length=length, method=method)
    return f.fgn()
