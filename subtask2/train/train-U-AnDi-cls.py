...
l = args.l
filename = './pp_data/data-cls-2d-{}-pp.csv'.format(l)
...
for idx, (train_idx, valid_idx) in enumerate(skf.split(data, data['N_diffmodel'])):
...
class AnDiDataset(Dataset):
    ...
    def __getitem__(self, index): 
        ...
        if self.label:            
            target = torch.Tensor([int(i) for i in self.df['mask'].iloc[index].split(' ')])
        else:
            target = 0
        return data_seq, torch.Tensor(target).long()
...
class AnDiModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, dilation_depth):
        ...
        # last conv
        self.conv = nn.Conv1d(hidden_dim, 5, 1)
...
model = AnDiModel(2, 128, 3, 5).to(device)
...
class DiceScore(nn.Module):
    ...
    def forward(self, logit, target):
        d1 = F.one_hot(torch.argmax(torch.softmax(logit, 1), 1), num_classes=self.num_classes)
        d2 = F.one_hot(target, num_classes=self.num_classes)
        return 2*(d1*d2).sum()/((d1+d2).sum())
...
def evaluate(step, history=None): 
    ...
    valid_status = 'Step: {}\tLR: {:.6f}\tValid Loss: {:.4f}\tValid Dice: {:.4f}'.format(
